...
class SalesBakso extends Component
{
    public $startDate;
    public $endDate;
    ...
    private function processSalesData()
    {
        $query = Transaction::with(['details.product'])
            ...
            ->whereHas('details.product', function ($q) {
                $q->where('category_id', 1); // Filter khusus untuk Kategori Bakso (ID 1)
            });
        ...
        foreach ($transactions as $transaction) {
            foreach ($transaction->details as $detail) {
                // Skip if product is not in category 1 (double check)
                if ($detail->product->category_id != 1) continue;
    ...
    {
        return view('livewire.reports.sales-bakso');
    }
}
